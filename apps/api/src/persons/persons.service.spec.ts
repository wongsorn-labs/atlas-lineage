...
describe('PersonsService', () => {
  ...
  it('findAll returns array', () => {
    expect(service.findAll(1)).toEqual([mockPerson]);
  });

  it('findOne returns person when found', async () => {
    await expect(service.findOne(1, 1)).resolves.toEqual(mockPerson);
  });
  ...
  it('create calls createPerson with dto', async () => {
    const dto = { treeId: 1, name: 'Grace Hopper' };
  ...
  it('update returns updated person', async () => {
    ...
    await expect(service.update(1, 1, dto as never)).resolves.toMatchObject({ id: 1, name: 'Ada Updated' });
  });

  it('update throws NotFoundException when not found', async () => {
    await expect(service.update(99, 1, { name: 'X' } as never)).rejects.toThrow(NotFoundException);
  });
