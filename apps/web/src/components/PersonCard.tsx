  ...
  return (
    <>
      <Card className="m-3">
        <CardHeader className="pb-2">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex gap-1 shrink-0">
              ...
              <Button size="icon" variant="ghost" className="h-7 w-7 text-red-500 hover:text-red-600" onClick={handleDelete}>
        ...
        <CardContent className="space-y-2">
          ...
          {myRels.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Relationships</p>
              {myRels.map((rel) => (
                <div key={rel.id} className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5">
                    <Badge variant={rel.type as RelationshipType}>{rel.type}</Badge>
        ...
        <CardFooter>
          <Button size="sm" variant="outline" className="w-full gap-1" onClick={() => setRelOpen(true)}>
      ...
      <Dialog open={relOpen} onOpenChange={setRelOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add relationship for {person.name}</DialogTitle>
          </DialogHeader>
          <RelationshipForm
